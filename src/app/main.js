...
let musicIndex = Math.floor((Math.random() * playlist.length) + 1);
// isMusicPaused = true;
//
...
function loadMusic(indexNumb){
    ...
    // musicImg.src = `images/${playlist[indexNumb - 1].src}.jpg`;
    mainAudio.src = `assets/audio/${playlist[indexNumb - 1].src}.mp3`;
}
//
// //play music function
function playMusic(){
...
mainAudio.addEventListener("ended", ()=>{
    ...
    switch(getText){
        ...
        case "repeat_one":
            ...
            break;
        case "shuffle":
            let randIndex = Math.floor((Math.random() * playlist.length) + 1); //genereting random index/numb with max range of array length
            do{
...
for (let i = 0; i < playlist.length; i++) {
    ...
    let liTag = `<li li-index="${i + 1}">
                ...
                <span id="${playlist[i].src}" class="audio-duration">3:40</span>
                <audio class="${playlist[i].src}" src="assets/audio/${playlist[i].src}.mp3"></audio>
              </li>`;
    ulTag.insertAdjacentHTML("beforeend", liTag); //inserting the li inside ul tag

    let liAudioDuartionTag = ulTag.querySelector(`#${playlist[i].src}`);
